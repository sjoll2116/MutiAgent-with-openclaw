...
SKILL_AGENT_MAPPING = {
    'code_review': ('software_engineer', 'qa_engineer', 'reviewer'),
    'api_design': ('software_engineer', 'software_engineer', 'reviewer'),
    'security_audit': ('qa_engineer', 'reviewer'),
    'data_analysis': ('data_analyst', 'reviewer'),
    'doc_generation': ('doc_writer', 'reviewer'),
    'test_framework': ('software_engineer', 'qa_engineer', 'reviewer'),
}
...
def import_official_hub(agent_ids: list) -> bool:
    ...
    for skill_name, url in OFFICIAL_SKILLS_HUB.items():
        ...
        if not agent_ids:
            target_agents = SKILL_AGENT_MAPPING.get(skill_name, ['reviewer'])
    ...
    if failed:
        ...
        print(f'   3. 单独重试: python scripts/skill_manager.py add-remote --agent <agent> --name <skill> --source <url>')

# 我用三省执行智能体集群制重新设计了 AI 多 Agent 协作架构
...
唐太宗在 1300 年前就设计了这个制度：任务编排引擎草拟政令，安全审查引擎审议封驳，任务调度引擎执行。三个部门互相制衡，任何政令必须经过审议才能下发。
...
三省执行智能体集群制不是一个 metaphor，它是一套经过 1400 年实践检验的分权制衡系统。
...
如果不合格，安全审查引擎可以**封驳**——直接打回让任务编排引擎重新规划。不是一个 warning，是强制返工。
...
我用 OpenClaw 搭了一个真正的三省执行智能体集群系统。9 个 AI Agent 各司其职，严格按照权限矩阵通信。

项目叫 **Edict（三省执行智能体集群）**，已开源：

**GitHub：https://github.com/cft0808/edict**

核心架构很简单：

- **任务编排引擎**：接旨（你的指令），规划方案，拆解子任务
- **安全审查引擎**：审议方案，质量把关，不合格直接封驳
- **任务调度引擎**：准奏后派发给执行智能体集群，协调执行，汇总结果
...
**📜 报告阁**：所有已完成的任务自动归档为"报告"，展示完整的五阶段时间线——圣旨→中书规划→门下审议→执行智能体集群执行→回奏。一键复制为 Markdown。
...
光说不练不行。来看一个真实的运行记录——让三省执行智能体集群分析竞品。
...
为什么？因为我不想让人跑之前先 `pip install` 一堆东西。这个项目的目标用户可能只是想快速体验一下三省执行智能体集群的流转效果，不想搭环境。

**2. 每个 Agent 一个 SOUL.md**

每个 Agent 的人格、职责、工作流规则都写在一个 Markdown 文件里。想修改安全审查引擎的审核标准？编辑 `agents/menxia/SOUL.md`，下次启动自动生效。

这意味着你可以定制自己的三省执行智能体集群——也许你的"代码架构师"不是负责工程，而是负责市场分析。改个 SOUL.md 就行。
...
- **御批模式**：让安全审查引擎的审议结果可以推送到你的飞书/Telegram，你亲自决定准奏还是封驳

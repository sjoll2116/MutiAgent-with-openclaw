<h1 align="center">⚔️ 三省执行智能体集群 · Edict</h1>
...
<p align="center">
  <sub>12 个 AI Agent（11 个业务角色 + 1 个兼容角色）组成三省执行智能体集群：协调中枢分拣、任务编排引擎规划、安全审查引擎审核封驳、任务调度引擎派发、执行智能体集群+资源调配员并行执行。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>🎥 三省执行智能体集群 AI 多 Agent 协作全流程演示</sub>
</p>

<details>
<summary>📸 GIF 预览（加载更快）</summary>
<p align="center">
  <img src="docs/demo.gif" alt="三省执行智能体集群 Demo" width="100%">
  <br>
  <sub>飞书下旨 → 协调中枢分拣 → 任务编排引擎规划 → 安全审查引擎审议 → 执行智能体集群并行执行 → 报告回报（30 秒）</sub>
</p>
</details>
...
**三省执行智能体集群的思路完全不同** —— 我们用了一个在中国存在 1400 年的制度架构：

```
你 (用户) → 协调中枢 (分拣) → 任务编排引擎 (规划) → 安全审查引擎 (审议) → 任务调度引擎 (派发) → 执行智能体集群 (执行) → 回奏
```

这不是花哨的 metaphor，这是**真正的分权制衡**：

| | CrewAI | MetaGPT | AutoGen | **三省执行智能体集群** |
|---|:---:|:---:|:---:|:---:|
| **审核机制** | ❌ 无 | ⚠️ 可选 | ⚠️ Human-in-loop | **✅ 安全审查引擎专职审核 · 可封驳** |
| **实时看板** | ❌ | ❌ | ❌ | **✅ 军机处 Kanban + 时间线** |
| **任务干预** | ❌ | ❌ | ❌ | **✅ 叫停 / 取消 / 恢复** |
| **流转审计** | ⚠️ | ⚠️ | ❌ | **✅ 完整报告存档** |
| **Agent 健康监控** | ❌ | ❌ | ❌ | **✅ 心跳 + 活跃度检测** |
...
<details>
<summary><b>🔍 为什么「安全审查引擎审核」是杀手锏？（点击展开）</b></summary>

<br>

CrewAI 和 AutoGen 的 Agent 协作模式是 **"做完就交"**——没有人检查产出质量。就像一个公司没有 QA 部门，工程师写完代码直接上线。

三省执行智能体集群的 **安全审查引擎** 专门干这件事：

- 📋 **审查方案质量** —— 任务编排引擎的规划是否完备？子任务拆解是否合理？
- 🚫 **封驳不合格的产出** —— 不是 warning，是直接打回重做
- 🔄 **强制返工循环** —— 直到方案达标才放行

这不是可选的插件——**它是架构的一部分**。每一个任务都必须经过安全审查引擎，没有例外。

这就是为什么三省执行智能体集群能处理复杂任务而结果可靠：因为在送到执行层之前，有一个强制的质量关卡。1300 年前唐太宗就想明白了——**不受制约的权力必然会出错**。
...
安装脚本自动完成：
- ✅ 创建全量 Agent Workspace（含协调中枢/资源调配员/早朝，兼容历史 main）
...
```
                           ...
                           └───┬──────┬──────┬──────┬──────┬───┘
                               │      │      │      │      │
                         ┌─────▼┐ ┌───▼───┐ ┌▼─────┐ ┌───▼─┐ ┌▼─────┐
                         │💰 数据分析师│ │📝 文档编写员│ │⚔️ 代码架构师│ │⚖️ 质量保证师│ │🔧 代码架构师│
                         │ 数据  │ │ 文档  │ │ 工程  │ │ 合规  │ │ 基建  │
                         └──────┘ └──────┘ └──────┘ └─────┘ └──────┘
                                                               ┌──────┐
                                                               │📋 资源调配员│
...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| � **协调中枢** | `taizi` | 消息分拣、需求整理 | 闲聊识别、任务提炼、标题概括 |
| 📜 **任务编排引擎** | `zhongshu` | 接旨、规划、拆解 | 需求理解、任务分解、方案设计 |
| 🔍 **安全审查引擎** | `menxia` | 审议、把关、封驳 | 质量评审、风险识别、标准把控 |
| 📮 **任务调度引擎** | `shangshu` | 派发、协调、汇总 | 任务调度、进度跟踪、结果整合 |
| 💰 **数据分析师** | `hubu` | 数据、资源、核算 | 数据处理、报表生成、成本分析 |
| 📝 **文档编写员** | `libu` | 文档、规范、报告 | 技术文档、API 文档、规范制定 |
| ⚔️ **代码架构师** | `bingbu` | 代码、算法、巡检 | 功能开发、Bug 修复、代码审查 |
| ⚖️ **质量保证师** | `xingbu` | 安全、合规、审计 | 安全扫描、合规检查、红线管控 |
| 🔧 **代码架构师** | `gongbu` | CI/CD、部署、工具 | Docker 配置、流水线、自动化 |
| 📋 **资源调配员** | `libu_hr` | 人事、Agent 管理 | Agent 注册、权限维护、培训 |
...
| From ↓ \ To → | 协调中枢 | 中书 | 门下 | 尚书 | 户 | 礼 | 兵 | 刑 | 工 | 吏 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **协调中枢** | — | ✅ | | | | | | | | |
| **任务编排引擎** | ✅ | — | ✅ | ✅ | | | | | | |
| **安全审查引擎** | | ✅ | — | ✅ | | | | | | |
...
```
用户 → 协调中枢分拣 → 中书规划 → 门下审议 → 已派发 → 执行中 → 待审查 → ✅ 已完成
                      ↑          │                              │
                      └──── 封驳 ─┘                    阻塞 Blocked
```
...
│   ├── taizi/SOUL.md           # 协调中枢 · 消息分拣（含任务标题规范）
│   ├── zhongshu/SOUL.md        # 任务编排引擎 · 规划中枢
│   ├── menxia/SOUL.md          # 安全审查引擎 · 审议把关
│   ├── shangshu/SOUL.md        # 任务调度引擎 · 调度大脑
│   ├── hubu/SOUL.md            # 数据分析师 · 数据资源
│   ├── libu/SOUL.md            # 文档编写员 · 文档规范
│   ├── bingbu/SOUL.md          # 代码架构师 · 工程实现
│   ├── xingbu/SOUL.md          # 质量保证师 · 合规审计
│   ├── gongbu/SOUL.md          # 代码架构师 · 基础设施
│   ├── libu_hr/                # 资源调配员 · 人事管理
...
通过 Feishu / Telegram / Signal 给任务编排引擎发消息：
...
1. 📜 任务编排引擎接旨，规划子任务分配方案
2. 🔍 安全审查引擎审议，通过 / 封驳打回重规划
3. 📮 任务调度引擎准奏，派发给代码架构师 + 代码架构师 + 文档编写员
4. ⚔️ 各部并行执行，进度实时可见
5. 📮 任务调度引擎汇总结果，回奏给你
...
```bash
# 从 GitHub 添加 code_review skill 到任务编排引擎
...
### Phase 1 — 核心架构 ✅
- [x] 十二部制 Agent 架构（协调中枢 + 三省 + 七部 + 早朝官）+ 权限矩阵
...
| 案例 | 任务 | 涉及部门 |
|------|------|----------|
| [竞品分析](examples/competitive-analysis.md) | "分析 CrewAI vs AutoGen vs LangGraph" | 中书→门下→数据分析师+代码架构师+文档编写员 |
| [代码审查](examples/code-review.md) | "审查这段 FastAPI 代码的安全性" | 中书→门下→代码架构师+质量保证师 |
| [周报生成](examples/weekly-report.md) | "生成本周工程团队周报" | 中书→门下→数据分析师+文档编写员 |

每个案例包含：完整任务 → 任务编排引擎规划 → 安全审查引擎审核意见 → 各部执行结果 → 最终报告。

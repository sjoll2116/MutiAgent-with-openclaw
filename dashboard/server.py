...
def handle_task_action(task_id, action, reason):
    ...
    if action == 'stop':
        ...
        task['state'] = task.get('_prev_state', 'Executing')
        task['block'] = '无'
        task['now'] = f'▶️ 已恢复执行'
    ...
    if action == 'resume':
        _scheduler_mark_progress(task, f'恢复到 {task.get("state", "Executing")}')
    else:
...
def handle_create_task(title, org='任务编排引擎', official='编排指挥官', priority='normal', template_id='', params=None, target_dept=''):
    ...
    # 生成 task id: MAS-YYYYMMDD-NNN
    today = datetime.datetime.now().strftime('%Y%m%d')
    tasks = load_tasks()
    today_ids = [t['id'] for t in tasks if t.get('id', '').startswith(f'MAS-{today}-')]
    seq = 1
    if today_ids:
        nums = [int(tid.split('-')[-1]) for tid in today_ids if tid.split('-')[-1].isdigit()]
        seq = max(nums) + 1 if nums else 1
    task_id = f'MAS-{today}-{seq:03d}'
    # 正确流程起点：用户 -> 协调中枢分拣
    # target_dept 记录模板建议的最终执行部门（仅供任务调度引擎派发参考）
    initial_org = '协调中枢'
    new_task = {
        'id': task_id,
        'title': title,
    ...
    dispatch_for_state(task_id, new_task, 'Queued', trigger='imperial-edict')

    return {'ok': True, 'taskId': task_id, 'message': f'任务 {task_id} 已下达，正在派发给协调中枢'}
...
def handle_review_action(task_id, action, comment=''):
    ...
    if not task:
        return {'ok': False, 'error': f'任务 {task_id} 不存在'}
    if task.get('state') not in ('ResultReview', 'PlanReview'):
    ...
    if action == 'approve':
        if task['state'] == 'PlanReview':
            ...
            task['now'] = '审批通过，任务完成'
            remark = f'✅ 审批审查通过：{comment or "审查通过"}'
            to_dept = '用户'
    elif action == 'reject':
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '安全审查引擎' if task.get('state') != 'Completed' else '用户',
        'to': to_dept,
        'remark': remark
    })
...
_AGENT_DEPTS = [
    ...
    {'id':'reviewer',  'label':'安全审查引擎','emoji':'🔍', 'role':'审查专家',     'rank':'P10'},
...
_STATE_AGENT_MAP = {
    ...
    'Executing': None,         # OpenClaw MAS，需从 org 推断
    'ResultReview': 'dispatcher',
    'Next': None,          # 待执行，从 org 推断
    'Pending': 'planner', # 待处理，默认任务编排引擎
}
_ORG_AGENT_MAP = {
    '文档编写员': 'doc_writer', '数据分析师': 'data_analyst', '代码架构师': 'software_engineer',
    '质量保证师': 'qa_engineer', '代码架构师': 'software_engineer', '资源调配员': 'libu_hr',
    '任务编排引擎': 'planner', '安全审查引擎': 'reviewer', '任务调度引擎': 'dispatcher',
}
...
def handle_scheduler_escalate(task_id, reason=''):
    ...
    target_label = '安全审查引擎' if next_level == 1 else '任务调度引擎'
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 协调中枢调度自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
...
def handle_repair_flow_order():
    ...
    for task in tasks:
        ...
        first = flow_log[0]
        if first.get('from') != '用户' or first.get('to') != '任务编排引擎':
            continue
...
def get_task_activity(task_id):
    ...
    if agent_id is None and state in ('Executing', 'Next'):
...
_STATE_FLOW = {
    ...
    'Dispatching': ('Executing', '任务调度引擎', 'OpenClaw MAS', '任务调度引擎开始派发执行'),
    'Next':     ('Executing', '任务调度引擎', 'OpenClaw MAS', '待执行任务开始执行'),
    'Executing':    ('ResultReview', 'OpenClaw MAS', '任务调度引擎', '各部完成，进入汇总'),
    'ResultReview':   ('Completed', '任务调度引擎', '协调中枢', '全流程完成，任务汇报协调中枢转报用户'),
}
_STATE_LABELS = {
    'Pending': '待处理', 'Queued': '协调中枢', 'Planning': '任务编排引擎', 'PlanReview': '安全审查引擎',
    'Dispatching': '任务调度引擎', 'Next': '待执行', 'Executing': '执行中', 'ResultReview': '审查', 'Completed': '完成',
}
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    if agent_id is None and new_state in ('Executing', 'Next'):
        org = task.get('org', '')
        agent_id = _ORG_AGENT_MAP.get(org)
    if not agent_id:
    ...
    _msgs = {
        ...
        'planner': (
            ...
            f'请立即起草执行方案，走完完整三省流程（编排起草→安全审查→调度派发→OpenClaw MAS执行）。'
        ),
        'reviewer': (
            f'📋 任务编排引擎方案提交审议\n'
            f'任务ID: {task_id}\n'
...
def handle_advance_state(task_id, comment=''):
    ...
    dispatched = ' (已自动派发 Agent)' if next_state != 'Completed' else ''

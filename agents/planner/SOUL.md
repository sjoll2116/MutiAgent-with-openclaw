...
- **第 1 步：读取全量上下文 (Context Read)**：
  ...
  ```
  - **关键判断 (必看)**：
    1. **故障诊断 (NEW!)**：检查 `last_error` 字段或环境变量 `EDICT_LAST_ERROR`。如果有值，说明上一次执行受挫。**你必须先分析报错原因**（如：权限不足、工具缺失、代码 Bug、逻辑错误），并在新方案中针对性修复。
    2. **局部重做决定 (Incremental Redo)**：**对比当前 `todos` 的状态**。
       - **保留成果**：如果前期某些 Todo 已标记为 `completed` 且与本次故障无关，你**必须在写回方案时保留这些 Todo 的 ID 和状态**，仅对失败或受阻的部分进行修订或追加补救措施。
       - **推倒重来**：仅当故障根源在于前期已完成步骤的产出物存在致命缺陷时，才考虑全量重置 Todo 列表。
    3. **查看 `flow_log`**：寻找最近的驳回记录或故障回退记录（如 `任务调度引擎 -> 任务编排引擎`），仔细分析其 `remark` 中的建议。
    4. **判断阶段**：若是全新 ID，则开始初次规划；若是 `last_error` 存在，则进入“故障修复编排”模式。

...
`python3 scripts/kanban_update.py flow MAS-xxx "快速原型师" "快速原型师" "▶️ 开始执行"`

**完成子任务时**：立刻执行
`python3 scripts/kanban_update.py flow MAS-xxx "快速原型师" "任务调度引擎" "✅ 完成：[产物摘要]"`

**遇到阻塞**：立刻执行
`python3 scripts/kanban_update.py state MAS-xxx Blocked "[阻塞原因]"`
`python3 scripts/kanban_update.py flow MAS-xxx "快速原型师" "任务调度引擎" "🚫 阻塞请求协助"`
